...
def sendPUT():
	...
	msg = f"PUT /PUT-TEST/{name}.txt HTTP/1.1{CRLF}Host: {SERVER}:{PORT}{CRLF}Content-Type: text/html{CRLF}Content-Length: {l}{CRLF}{CRLF}{data}"
	return msg
...
def sendDELETE():
	...
	name = ''.join(random.SystemRandom().choice(string.ascii_lowercase + string.digits) for _ in range(6))
	data = ''.join(random.SystemRandom().choice(string.ascii_lowercase + string.digits) for _ in range(l))
	p4 = f"/DELETE-TEST/{name}.txt"
	
	actual_path = BASE_DIR + f"/var/www/html{p4}"

...
def use_logger(access=True, post=False, error=False):
	if post:
		print("IN POST")
		# filename = 	
		LOG_FORMAT = "%(levelname)s %(asctime)s : %(message)s"
		logging.basicConfig(filename=f"{BASE_DIR}/var/log/Atharva-Server/post.log", level=logging.DEBUG, format=LOG_FORMAT)
		return logging.getLogger()

	elif error:
		# print('\n\nERROR RECEIVED\n\n')
		# filename = f
		logging.basicConfig(filename=f"{BASE_DIR}/var/log/Atharva-Server/error.log", level=logging.DEBUG, format='[%(asctime)s] %(message)s', datefmt='%a %b %d %H:%M:%S %Y')
		return logging.getLogger()
		# [Wed Nov 11 12:31:58.467192 2020]
		# [%a %b %d %H:%M:%S %Y]
	elif access:
		filename = f"{BASE_DIR}/var/log/Atharva-Server/access.log"
...
class RequestHandler():
	...
	def setmethod(self, msg):
		...
		if path == "/":
			path = BASE_DIR + "/var/www/html/index.html"
		
		elif path == "/favicon.ico":
			path = BASE_DIR + "/var/www/html/Images/favicon.ico"
		
		else:
			path = BASE_DIR + "/var/www/html" + path
		
		if os.path.exists(path) or self.method == "PUT" or path in PERMANENTLY_MOVED:
			# print("STEP 6")
	...
	def logdata(self, err_msg = None, err_resp = None):
		if not (err_msg is None and err_resp is None):
			logger = use_logger(access=False, post=False, error=True)
		...
		logger.info(msg)
		if self.method == "POST":
			logger = use_logger(access=False, post=True, error=False)
	...
	def check_cookie(self):
		try:
			...
			for cookie in cookies:
				if cookie.startswith('atharvas_cookie'):
					cookie = cookie
					break
			print(f"COOKIE_CHECKING: {cookie}")
	...
	def cookie_handler(self):
		...
		if check is None:
			...
			t_cookie = "Set-Cookie: atharvas_cookie=" + self.create_cookie() + CRLF
